...
var IndentGuideRenderer = class {
  constructor(tree, provider) {
    this.tree = tree;
    const onDidRender = tree.onDidRefrash;
  ...
  async render(revision) {
    ...
    try {
      ...
      const explorerConfig = import_coc.workspace.getConfiguration("explorer");
      const enabled = explorerConfig.get(
        "indentGuides.enabled",
        true
      );
      const character = explorerConfig.get(
        "indentGuides.character",
        "\u2502"
      );
      const treeConfig = import_coc.workspace.getConfiguration("tree");
      const openedIcon = treeConfig.get("openedIcon", " ");
      const closedIcon = treeConfig.get("closedIcon", " ");
  ...
  async getNamespace() {
    if (this.namespace === void 0) {
      this.namespace = await import_coc.workspace.nvim.call("nvim_create_namespace", [
...
var Explorer = class {
  constructor(ui, context) {
    ...
    context.subscriptions.push(
      ...
      import_coc.events.on("WinEnter", (winid) => this.onWindowFocused(winid)),
  ...
  rememberCurrentFile(bufnr) {
    ...
    this.currentFile = uri.scheme === "file" ? uri.fsPath : void 0;
  }
  ...
  async revealFile(filename, ensureVisible) {
...
function renderedDepth(line, openedIcon, closedIcon) {
  const indentation = line.match(/^ */)?.[0].length ?? 0;
  if (indentation === 0) return 0;
  const content = line.slice(indentation);
  const hasVisibleTreeIcon = [openedIcon, closedIcon].filter((icon) => icon.trim().length > 0).some((icon) => content.startsWith(`${icon} `));
