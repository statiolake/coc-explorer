...
var TreeDecorationRenderer = class {
  ...
  async render(revision) {
    ...
    try {
      ...
      const folderClosed = explorerConfig.get(
        "icons.folderClosed",
        "\uF460"
      );
      const folderOpen = explorerConfig.get("icons.folderOpen", "\uF47C");
      const fileIcon = explorerConfig.get("icons.file", "\uF15B");
      const renderAwareTree = this.tree;
      ...
      for (let index = 0; index < renderedItems.length; index += 1) {
        ...
        const marker = node.directory ? this.provider.isExpanded(node) ? folderOpen : folderClosed : fileIcon;
        if (!marker) continue;
        nvim.call(
          "nvim_buf_set_extmark",
